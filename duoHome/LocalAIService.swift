    ...
    func sendMessage(prompt: String, completion: @escaping CompletionHandler) {
        ...
        sendRequest(requestBody: requestBody, isStreaming: false) { data, error in
            ...
            guard let data = data else {
                completion(nil, NSError(domain: "LocalAIService", code: 1, userInfo: [NSLocalizedDescriptionKey: "没有返回数据"]))
                return
            }
            ...
            do {
                if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
                   ...
                   let content = message["content"] as? String {
                    ...
                    completion(nil, NSError(domain: "LocalAIService", code: 2, userInfo: [NSLocalizedDescriptionKey: "解析响应失败"]))
                }
            } catch {
                completion(nil, error)
            }
